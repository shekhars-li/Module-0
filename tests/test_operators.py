...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert(sig > 0)
    assert(sig <= 1.0)
    assert_close(neg_sig, 1.0 - sigmoid(a))
    assert(sigmoid(-1e-5) < 0.5)
    assert(sigmoid(1e-5) > 0.5)
    assert(sig <= sigmoid(a + 1e5))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    a, b, c, d, e = addLists(ls1, ls2)
    assert(a == ls1[0] + ls2[0])
    assert(b == ls1[1] + ls2[1])
    assert(c == ls1[2] + ls2[2])
    assert(d == ls1[3] + ls2[3])
    assert(e == ls1[4] + ls2[4])
